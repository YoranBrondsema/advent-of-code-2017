defmodule Day2 do
  def checksum1(s) do
    to_rows_of_integers(s)
    |> Enum.map(&diff_between_largest_and_smallest/1)
    |> Enum.sum
  end

  def checksum2(s) do
    to_rows_of_integers(s)
    |> Enum.map(&get_dividable_elements/1)
    |> Enum.sort
    |> Enum.map(
      fn([ max | [min | []]]) -> div(max, min) end
    )
    |> Enum.sum
  end

  defp get_dividable_elements(l) do
    List.first(
      for i <- l, j <- l,
        rem(i, j) == 0 && i != j,
        do: [i, j]
  ...
  defp diff_between_largest_and_smallest(l) do
...
Enum.each(
  ...
  fn(x) ->
    IO.puts x
    IO.puts "checksum1: #{Day2.checksum1(x)}"
    IO.puts "checksum2: #{Day2.checksum2(x)}"
  end
)
